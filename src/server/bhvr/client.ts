...
/**
 * Calls POST /api/v1/matchIncentives for one region at a time. Sends exactly the
 * header set a real client sends, reuses a single api-key, and re-authenticates
 * only on a 401 (then retries the call once).
 */
export class BhvrClient {
  ...
  private async doFetch(
    ...
    const headers: Record<string, string> = {
      ...
      'x-kraken-client-version': this.options.clientVersion,
