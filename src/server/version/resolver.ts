...
export class VersionUnresolvedError extends Error {
  override name = 'VersionUnresolvedError';
}
...
/**
 * Owns which client version (and therefore category + User-Agent) is currently
 * used. In auto mode it discovers the newest public build and applies the
 * cosmetic-patch guard: try the freshly discovered category, but if a full poll
 * pass returns only fallbacks, fall back to the last category that returned real
 * data and wait for a newer build before trying again.
 */
